...
# EXPANDED safety list - files that might be important
$neverMove = @(
    "*package*.json",
    "main.js",
    "preload.js",
...
function Move-FileToExile {
    ...
    foreach ($pattern in $neverMove) {
        if ($RelativePath -like $pattern) {
            Write-Host "  SKIP: Protected file - $RelativePath" -ForegroundColor Red
...
foreach ($category in $categories) {
    ...
    foreach ($file in $category.Files) {
        if (Move-FileToExile -RelativePath $file) {
            $totalWouldMove++
        } else {
            $totalWouldSkip++
        }
...
if (-not $WhatIf -and $movedFiles.Count -gt 0) {
    ...
    Write-Host "Would move: $totalWouldMove files" -ForegroundColor Green
    Write-Host "Would skip: $totalWouldSkip files" -ForegroundColor Red
